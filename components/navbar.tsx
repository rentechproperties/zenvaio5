...
const industries = [
  { name: 'Real Estate', slug: 'real-estate', desc: 'Agent & Broker Solutions' },
  { name: 'Healthcare', slug: 'healthcare', desc: 'Patient Booking' },
  { name: 'Home Services', slug: 'home-services', desc: 'Plumbing, HVAC, Electrical' },
  { name: 'Auto Dealers', slug: 'auto-dealers', desc: 'Sales & Service' },
]
        ...
        <div className="flex h-20 items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Link href="/" className="flex items-center gap-2">
              ...
              <span className={`text-xl font-bold tracking-tight ${isScrolled ? 'text-[#0F172A]' : 'text-white'}`}>
                Zenvaio
              </span>
            </Link>
          </div>
          ...
          <div className="hidden lg:flex lg:items-center lg:gap-8">
            ...
            <div className="relative" onMouseEnter={() => setIndustriesOpen(true)} onMouseLeave={() => setIndustriesOpen(false)}>
              <button className={`flex items-center gap-1 text-sm font-medium transition-colors ${
                  isScrolled ? 'text-[#475569] hover:text-[#0F172A]' : 'text-white/80 hover:text-white'
                }`}>
                Industries <ChevronDown className="h-4 w-4" />
              </button>
              {industriesOpen && (
                <div className="absolute left-0 top-full mt-2 w-64 rounded-xl border border-[#E2E8F0] bg-white p-2 shadow-xl">
                  <div className="grid gap-1">
                    {industries.map((ind) => (
                      <Link key={ind.slug} href="#industry-selector" className="block rounded-lg p-3 hover:bg-[#F8FAFC]">
          ...
          <div className="hidden items-center gap-3 lg:flex">
            {/* This is the fixed Desktop ROI Link */}
            <Link
              href="#roi-calculator"
      ...
      {isMobileMenuOpen && (
        <div className="animate-fadeIn border-t border-[#E2E8F0] bg-white lg:hidden">
          <div className="space-y-1 px-4 py-4">
            ...
            {/* This is the fixed Mobile ROI Link */}
            <Link
              href="#roi-calculator"
              onClick={() => setIsMobileMenuOpen(false)}
